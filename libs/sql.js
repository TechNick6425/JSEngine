var db = (function () {
		...
		sqlProcessor = function (transaction, sqlObjects, finalSuccess, failure) {
			...
			var sequenceNumber = 0,
				...
				successCallback = function (transaction, results) {
					...
					if(db.putSelectResultsInArray && isSelectStmt(sqlObjects[sequenceNumber].sql)){
	...
	return {
		...
		openDatabase: function (name, displayname, size, whenOpen) {
			db.database = openDatabase(name, "", displayname, size);
			readTransactionAvailable = typeof db.database.readTransaction === 'function';
			if (whenOpen) {
				whenOpen();
			}
		},
		...
		/* This is the same as db.process but used when you want to change the
		 ...
		 */
			if (db.database) {
				if(db.database.version === oldVersion){
					var sqlObjects = sqlObjectCreator(sqlInput);
					...
					db.database.changeVersion(oldVersion, newVersion, function (transaction) {
						sqlProcessor(transaction, sqlObjects, finalSuccessCallback, failureCallback);
					});
				}
			} else {
